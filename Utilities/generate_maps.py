...
for mk, mv in mei_chars.items():
    for uk, uv in uyir_chars.items():
        ...
        parser[tam_chars[char_iter]] = (mk,uk)
        char_iter += 1
...
# with open('data/char_source_combinations.json', 'w',encoding='utf8') as outfile:
#     json.dump(combinations, outfile,ensure_ascii=False,indent=2)

with open('data/char_parser.json', 'w',encoding='utf8') as outfile:
    json.dump(parser, outfile,ensure_ascii=False,indent=2)
